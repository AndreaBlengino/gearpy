...
class TestSolverRun:
    ...
    @mark.genuine
    @given(
        time_discretization_1=time_intervals(),
    ...
    def test_method(
        self,
        time_discretization_1,
        time_discretization_multiplier,
        simulation_steps_1,
        simulation_steps_2,
        powertrain,
        ...
        powertrain.elements[-1].external_torque = \
            lambda time, angular_position, angular_speed: Torque(0.001, 'Nm')
        ...
        solver.run(
            time_discretization=time_discretization_1,
        ...
        if encoder.get_value() >= threshold:
            stopped = True
            assert len(powertrain.time) <= len(
                np.arange(
        ...
        time_discretization_2 = time_discretization_1 * \
            time_discretization_multiplier
        simulation_time_2 = time_discretization_2*simulation_steps_2
        solver.run(
            time_discretization=time_discretization_2,
            simulation_time=simulation_time_2,
            motor_control=motor_control
        )
    ...
    def test_raises_type_error(self, solver_run_type_error):
        if solver_run_type_error:
            ...
            gear.external_torque = \
                lambda angular_position, angular_speed, time: 1
