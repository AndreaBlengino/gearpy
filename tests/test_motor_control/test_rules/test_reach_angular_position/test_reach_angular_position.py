...
from tests.test_motor_control.test_rules.test_reach_angular_position.\
    conftest import PowertrainFake
...
class TestReachAngularPositionInit:
    ...
    def test_method(
        ...
        assert rule._ReachAngularPosition__target_angular_position == \
            target_angular_position
...
class TestReachAngularPositionApply:
    ...
    @given(
        ...
        braking_angle_multiplier=floats(
            allow_nan=False,
            allow_infinity=False,
            min_value=2,
            max_value=1000
    ...
    def test_method(
        ...
        braking_angle_multiplier,
        target_angular_position_multiplier,
        available_load_torque
    ):
        element_index %= len(powertrain.elements)
        encoder = AbsoluteRotaryEncoder(
            target=powertrain.elements[element_index]
        ...
        rule = ReachAngularPosition(
            ...
            braking_angle=Angle(
                value=current_angular_position*braking_angle_multiplier,
                unit='rad'
            )
        )
        powertrain.elements[element_index].angular_position = AngularPosition(
            value=current_angular_position,
            unit='rad'
        )
