...
class TestStartProportionalToAngularPositionInit:
    ...
    def test_method(
        ...
        encoder = AbsoluteRotaryEncoder(
            target=powertrain.elements[element_index]
        )
        rule = StartProportionalToAngularPosition(
            encoder=encoder,
        ...
        assert rule._StartProportionalToAngularPosition__powertrain == \
            powertrain
        assert rule.\
            _StartProportionalToAngularPosition__target_angular_position == \
            target_angular_position
        assert rule.\
            _StartProportionalToAngularPosition__pwm_min_multiplier == \
            pwm_min_multiplier
    ...
    def test_raises_type_error(
        self,
        start_proportional_to_angular_position_init_type_error
    ):
    ...
    def test_raises_value_error(
        self,
        start_proportional_to_angular_position_init_value_error
    ):
...
class TestStartProportionalToAngularPositionApply:
    ...
    @given(
        ...
        current_angular_position=floats(
            allow_nan=False,
            allow_infinity=False,
            min_value=1,
            max_value=1000
    ...
    def test_method(
        ...
        if powertrain.elements[0].electric_current_is_computable:
            if powertrain.elements[0].no_load_electric_current.value > 0:
                ...
                rule = StartProportionalToAngularPosition(
                    ...
                    target_angular_position=AngularPosition(
                        value=current_angular_position *
                        target_angular_position_multiplier,
                        unit='rad'
                    ),
                    pwm_min_multiplier=pwm_min_multiplier,
                ...
                powertrain.elements[element_index].angular_position = \
                    AngularPosition(value=current_angular_position, unit='rad')
            ...
            motor.angular_position = AngularPosition(
                value=current_angular_position,
                unit='rad'
            )
