...
from gearpy.motor_control.rules import (
    StartLimitCurrent,
...
class TestPWMControlAddRule:
    ...
    def test_method(
        self,
        powertrain,
        start_target_angular_position,
        reach_target_angular_position,
        pwm_min_multiplier,
        pwm_min,
        braking_angle,
        limit_electric_current,
        element_index,
        use_limit_current_rule
    ):
        ...
        reach_rule = ReachAngularPosition(
            encoder=encoder,
            powertrain=powertrain,
...
class TestPWMControlApplyRules:
    ...
    def test_method(
        ...
        if use_limit_current_rule:
            tachometer = Tachometer(target=powertrain.elements[element_index])
            powertrain.elements[element_index].angular_speed = \
                current_angular_speed
            start_rule = StartLimitCurrent(
                encoder=encoder,
                tachometer=tachometer,
            ...
            if powertrain.elements[0].time_variables['load torque']:
                powertrain.elements[0].time_variables['load torque'][0] = \
                    Torque(0, 'Nm')
            else:
