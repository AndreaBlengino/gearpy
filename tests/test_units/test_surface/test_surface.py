...
class TestSurfaceFormat:
    ...
    def test_method(self, value, unit, total_digits, decimal_digits):
        ...
        assert surface.__format__(
            f'{total_digits}.{decimal_digits}f'
        ) == f'{surface:{total_digits}.{decimal_digits}f}'
...
class TestSurfaceSub:
    ...
    def test_raises_value_error(self):
        with raises(ValueError):
            assert basic_surface - \
                Surface(basic_surface.value + 1, basic_surface.unit)
...
class TestSurfaceTruediv:
    ...
    def test_raises_zero_division_error(
        self,
        surface_truediv_zero_division_error
    ):
...
class TestSurfaceTo:
    ...
    def test_method(self, value, unit, inplace):
        if abs(value) >= 1e-10:
            ...
            for target_unit in units_list:
                converted_inertia = surface.to(
                    target_unit=target_unit,
                    inplace=inplace
                )

                assert converted_inertia.unit == target_unit
                if Surface._Surface__UNITS[target_unit] != \
                        Surface._Surface__UNITS[unit]:
