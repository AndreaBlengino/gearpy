...
class TestAngleInit:
    ...
    @mark.genuine
    @given(
        value=floats(allow_nan=False, allow_infinity=False, min_value=0),
...
class TestLengthFormat:
    ...
    def test_method(self, value, unit, total_digits, decimal_digits):
        ...
        assert angle.__format__(f'{total_digits}.{decimal_digits}f') == \
            f'{angle:{total_digits}.{decimal_digits}f}'
...
class TestAngleAdd:
    ...
    @mark.genuine
    @given(
        value_1=floats(allow_nan=False, allow_infinity=False, min_value=0),
...
class TestAngleTruediv:
    ...
    def test_raises_zero_division_error(
        self,
        angle_truediv_zero_division_error
    ):
...
class TestAngleTo:
    ...
    def test_method(self, value, unit, inplace):
        if abs(value) >= 1e-10:
            ...
            for target_unit in units_list:
                converted_angle = angle.to(
                    target_unit=target_unit,
                    inplace=inplace
                )

                assert converted_angle.unit == target_unit
                if Angle._AngularPosition__UNITS[target_unit] != \
                        Angle._AngularPosition__UNITS[unit]:
