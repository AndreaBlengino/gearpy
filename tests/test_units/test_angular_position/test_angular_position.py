...
from tests.test_units.test_angular_position.conftest import (
    basic_angular_position,
    angular_positions
)
...
class TestAngularPositionFormat:
    ...
    def test_method(self, value, unit, total_digits, decimal_digits):
        ...
        assert angular_position.__format__(
            f'{total_digits}.{decimal_digits}f'
        ) == f'{angular_position:{total_digits}.{decimal_digits}f}'
...
class TestAngularPositionAbs:
    ...
    def test_method(self, value, unit):
        ...
        assert abs(angular_position) == AngularPosition(
            value=abs(value),
            unit=unit
        )
...
class TestAngularPositionAdd:
    ...
    def test_method(self, value_1, unit_1, value_2, unit_2):
        ...
        assert result.value == angular_position_1.value + \
            angular_position_2.to(unit_1).value
...
class TestAngularPositionTruediv:
    ...
    def test_raises_zero_division_error(
        self,
        angular_position_truediv_zero_division_error
    ):
        with raises(ZeroDivisionError):
            assert basic_angular_position / \
                angular_position_truediv_zero_division_error
...
class TestAngularPositionTo:
    ...
    def test_method(self, value, unit, inplace):
        if abs(value) >= 1e-10:
            ...
            for target_unit in units_list:
                converted_position = angular_position.to(
                    target_unit=target_unit,
                    inplace=inplace
                )

                assert converted_position.unit == target_unit
                if AngularPosition._AngularPosition__UNITS[target_unit] != \
                        AngularPosition._AngularPosition__UNITS[unit]:
                    assert converted_position.value != value
                    assert converted_position.unit != unit

                    if inplace:
                        assert converted_position.value == \
                            angular_position.value
