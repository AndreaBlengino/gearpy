...
class TestCurrentTruediv:
    ...
    @mark.error
    def test_raises_zero_division_error(self, current_truediv_zero_division_error):
...
class TestCurrentTo:
    ...
    def test_method(self, value, unit, inplace):
        if abs(value) >= 1e-10:
            ...
            for target_unit in units_list:
                converted_current = current.to(target_unit = target_unit, inplace = inplace)

                assert converted_current.unit == target_unit
                if Current._Current__UNITS[target_unit] != Current._Current__UNITS[unit]:
