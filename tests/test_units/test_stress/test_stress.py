...
class TestStressFormat:
    ...
    def test_method(self, value, unit, total_digits, decimal_digits):
        ...
        assert stress.__format__(
            f'{total_digits}.{decimal_digits}f'
        ) == f'{stress:{total_digits}.{decimal_digits}f}'
...
class TestStressTruediv:
    ...
    def test_raises_zero_division_error(
        self,
        stress_truediv_zero_division_error
    ):
...
class TestStressTo:
    ...
    def test_method(self, value, unit, inplace):
        if abs(value) >= 1e-10:
            ...
            for target_unit in units_list:
                converted_stress = stress.to(
                    target_unit=target_unit,
                    inplace=inplace
                )

                assert converted_stress.unit == target_unit
                if Stress._Stress__UNITS[target_unit] != \
                        Stress._Stress__UNITS[unit]:
