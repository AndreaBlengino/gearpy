...
class TestTimeIntervalFormat:
    ...
    def test_method(self, value, unit, total_digits, decimal_digits):
        ...
        assert time_interval.__format__(
            f'{total_digits}.{decimal_digits}f'
        ) == f'{time_interval:{total_digits}.{decimal_digits}f}'
...
class TestTimeIntervalAdd:
    ...
    def test_method(self, value_1, unit_1, value_2, unit_2):
        ...
        assert result_1.value == time_interval_1.value + \
            time_interval_2.to(unit_1).value
...
class TestTimeIntervalSub:
    ...
    def test_method(self, value_1, unit_1, value_2, unit_2):
        ...
        if time_interval_1 > time:
            ...
            assert isinstance(result_2, Time)
            assert result_2.value == time_interval_1.value + \
                time.to(unit_1).value
            assert result_2.unit == time_interval_1.unit
    ...
    def test_raises_value_error(self):
        with raises(ValueError):
            assert basic_time_interval - TimeInterval(
                basic_time_interval.value + 1,
                basic_time_interval.unit
            )
...
class TestTimeIntervalTruediv:
    ...
    def test_raises_zero_division_error(
        self,
        time_interval_truediv_zero_division_error
    ):
        with raises(ZeroDivisionError):
            assert basic_time_interval / \
                time_interval_truediv_zero_division_error
...
class TestTimeIntervalTo:
    ...
    def test_method(self, value, unit, inplace):
        if abs(value) >= 1e-10:
            ...
            for target_unit in units_list:
                converted_time_interval = time_interval.to(
                    target_unit=target_unit,
                    inplace=inplace
                )

                assert converted_time_interval.unit == target_unit
                if TimeInterval._Time__UNITS[target_unit] != \
                        TimeInterval._Time__UNITS[unit]:
                    assert converted_time_interval.value != value
                    assert converted_time_interval.unit != unit

                    if inplace:
                        assert converted_time_interval.value == \
                            time_interval.value
                        assert converted_time_interval.unit == \
                            time_interval.unit
                    else:
                        assert converted_time_interval.value != \
                            time_interval.value
