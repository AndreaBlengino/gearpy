...
class TestTimeInit:
    ...
    @mark.genuine
    @given(
        value=floats(allow_nan=False, allow_infinity=False),
...
class TestTimeFormat:
    ...
    def test_method(self, value, unit, total_digits, decimal_digits):
        ...
        assert time.__format__(
            f'{total_digits}.{decimal_digits}f'
        ) == f'{time:{total_digits}.{decimal_digits}f}'
...
class TestTimeAbs:
    ...
    def test_method(self, value, unit):
        ...
        assert abs(time) == Time(value=abs(value), unit=unit)
...
class TestTimeNeg:
    ...
    def test_method(self, value, unit):
        ...
        assert -time == Time(value=-value, unit=unit)
...
class TestTimeRmul:
    ...
    def test_method_patch_1(self, value, unit):
        ...
        assert isinstance(result, AngularSpeed)
        assert result.value == time.to('sec').value * \
            fake_multiplier.to('rad/s^2').value
        assert result.unit == 'rad/s'
    ...
    def test_method_patch_2(self, value, unit):
        ...
        assert isinstance(result, AngularPosition)
        assert result.value == time.to('sec').value * \
            fake_multiplier.to('rad/s').value
        assert result.unit == 'rad'
...
class TestTimeTruediv:
    ...
    def test_raises_zero_division_error(
        self,
        time_truediv_zero_division_error
    ):
