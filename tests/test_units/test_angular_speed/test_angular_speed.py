...
from tests.test_units.test_angular_speed.conftest import (
    basic_angular_speed,
    angular_speeds,
    times
)
...
class TestAngularSpeedFormat:
    ...
    def test_method(self, value, unit, total_digits, decimal_digits):
        ...
        assert angular_speed.__format__(
            f'{total_digits}.{decimal_digits}f'
        ) == f'{angular_speed:{total_digits}.{decimal_digits}f}'


@mark.units
class TestAngularSpeedAbs:
    ...
    def test_method(self, value, unit):
        ...
        assert abs(angular_speed) == AngularSpeed(value=abs(value), unit=unit)
        assert abs(angular_speed).value >= 0


@mark.units
class TestAngularSpeedNeg:
    ...
    def test_method(self, value, unit):
        ...
        assert -angular_speed == AngularSpeed(value=-value, unit=unit)
...
class TestAngularSpeedAdd:
    ...
    def test_method(self, value_1, unit_1, value_2, unit_2):
        ...
        assert result.value == angular_speed_1.value + \
            angular_speed_2.to(unit_1).value
...
class TestAngularSpeedMul:
    ...
    def test_method(self, value, unit, multiplier):
        ...
        if isinstance(multiplier, Time):
            ...
            assert result.value == angular_speed.to('rad/s').value * \
...
class TestAngularSpeedTruediv:
    ...
    def test_raises_zero_division_error(
        self,
        angular_speed_truediv_zero_division_error
    ):
        with raises(ZeroDivisionError):
            assert basic_angular_speed / \
                angular_speed_truediv_zero_division_error
...
class TestAngularSpeedTo:
    ...
    def test_method(self, value, unit, inplace):
        if abs(value) >= 1e-10:
            ...
            for target_unit in units_list:
                converted_speed = angular_speed.to(
                    target_unit=target_unit,
                    inplace=inplace
                )

                assert converted_speed.unit == target_unit
                if AngularSpeed._AngularSpeed__UNITS[target_unit] != \
                        AngularSpeed._AngularSpeed__UNITS[unit]:
