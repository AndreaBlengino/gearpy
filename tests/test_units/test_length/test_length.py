...
class TestLengthSub:
    ...
    def test_raises_value_error(self):
        with raises(ValueError):
            assert basic_length - Length(basic_length.value + 1, basic_length.unit)
...
class TestLengthTruediv:
    ...
    @mark.error
    def test_raises_zero_division_error(self, length_truediv_zero_division_error):
...
class TestLengthTo:
    ...
    def test_method(self, value, unit, inplace):
        if abs(value) >= 1e-10:
            ...
            for target_unit in units_list:
                converted_inertia = length.to(target_unit = target_unit, inplace = inplace)

                assert converted_inertia.unit == target_unit
                if Length._Length__UNITS[target_unit] != Length._Length__UNITS[unit]:
