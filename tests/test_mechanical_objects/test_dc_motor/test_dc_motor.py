...
class TestDCMotorDrives:
    ...
    def test_property(self):
        gear = SpurGear(
            ...
            n_teeth=10,
            module=Length(1, 'mm'),
            inertia_moment=basic_inertia_moment
        )
    ...
    def test_method(
        ...
        if electric_motor:
            motor = DCMotor(
                ...
                no_load_speed=no_load_speed,
                maximum_torque=maximum_torque,
                no_load_electric_current=no_load_electric_current,
...
class TestDCMotorElectricCurrent:
    ...
    def test_raises_type_error(self, dc_motor_electric_current_type_error):
        with raises(TypeError):
            basic_dc_motor_1.electric_current = \
                dc_motor_electric_current_type_error
...
class TestDCMotorElectricCurrentIsComputable:
    ...
    def test_property(self):
        for motor in [basic_dc_motor_1, basic_dc_motor_2]:
            if (motor.no_load_electric_current is None) or \
                    (motor.maximum_electric_current is None):
