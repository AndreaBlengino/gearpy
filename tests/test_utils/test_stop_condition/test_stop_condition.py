...
threshold_map = {
    basic_amperometer: Current(1, 'A'),
    basic_encoder: AngularPosition(1, 'rot'),
...
class TestStopConditionInit:
    ...
    @given(
        sensor=sampled_from(
            elements=[basic_amperometer, basic_encoder, basic_tachometer]
        ),
        operator=sampled_from(
            elements=[
                StopCondition.greater_than,
                StopCondition.greater_than_or_equal_to,
                StopCondition.equal_to,
                StopCondition.less_than,
...
class TestStopConditionCheckCondition:
    ...
    @mark.genuine
    @given(
        threshold=one_of(angular_positions(), angular_speeds(), currents()),
        operator=sampled_from(
            elements=[
                StopCondition.greater_than,
                StopCondition.greater_than_or_equal_to,
    ...
    def test_raises_type_error(
        self,
        stop_condition_check_condition_type_error
    ):
        for operator in [
            StopCondition.greater_than,
            StopCondition.greater_than_or_equal_to,
