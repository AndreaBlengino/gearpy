...
from gearpy.units import Force, InertiaMoment, Length
from pytest import fixture
from tests.conftest import types_to_check
from typing import Callable


spur_gear_init_type_error_1 = [{'name': type_to_check, 'n_teeth': 10, 'module': Length(1, 'mm'),
                                'inertia_moment': InertiaMoment(1, 'kgm^2')}
                               for type_to_check in types_to_check if not isinstance(type_to_check, str)]

spur_gear_init_type_error_2 = [{'name': 'gear', 'n_teeth': type_to_check, 'module': Length(1, 'mm'),
                                'inertia_moment': InertiaMoment(1, 'kgm^2')}
                               for type_to_check in types_to_check if not isinstance(type_to_check, int)
                               and not isinstance(type_to_check, bool)]

spur_gear_init_type_error_3 = [{'name': 'gear', 'n_teeth': 10, 'module': type_to_check,
                                'inertia_moment': InertiaMoment(1, 'kgm^2')}
                               for type_to_check in types_to_check if not isinstance(type_to_check, Length)]

spur_gear_init_type_error_4 = [{'name': 'gear', 'n_teeth': 10, 'module': Length(1, 'mm'), 'inertia_moment': type_to_check}
...
@fixture(params = [{'name': '', 'n_teeth': 1, 'module': Length(1, 'mm'), 'inertia_moment': InertiaMoment(1, 'kgm^2')},
                   {'name': 'gear', 'n_teeth': -1, 'module': Length(1, 'mm'), 'inertia_moment': InertiaMoment(1, 'kgm^2')}])
