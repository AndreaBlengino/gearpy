...
class TestSpurGearDrivenBy:
    ...
    def test_property(self):
        ...
        for master in [motor, gear]:
            basic_spur_gear_1.driven_by = master

            assert basic_spur_gear_1.driven_by == master


    @mark.error
    def test_raises_type_error(self, spur_gear_driven_by_type_error):
        with raises(TypeError):
            basic_spur_gear_1.driven_by = spur_gear_driven_by_type_error
...
class TestSpurGearDrives:
    ...
    def test_property(self):
        gear = SpurGear(name = 'gear', n_teeth = 10, inertia_moment = InertiaMoment(1, 'kgm^2'))
        basic_spur_gear_1.drives = gear

        assert basic_spur_gear_1.drives == gear


    @mark.error
    def test_raises_type_error(self, spur_gear_drives_type_error):
        with raises(TypeError):
            basic_spur_gear_1.drives = spur_gear_drives_type_error
...
class TestSpurGearMasterGearRatio:
    ...
    def test_property(self, master_gear_ratio):
        basic_spur_gear_1.master_gear_ratio = master_gear_ratio

        assert basic_spur_gear_1.master_gear_ratio == master_gear_ratio


    @mark.error
    def test_raises_type_error(self, spur_gear_master_gear_ratio_type_error):
        with raises(TypeError):
            basic_spur_gear_1.master_gear_ratio = spur_gear_master_gear_ratio_type_error


    @mark.error
    def test_raises_value_error(self):
        with raises(ValueError):
            basic_spur_gear_1.master_gear_ratio = -1.0
...
class TestSpurGearMasterGearEfficiency:
    ...
    def test_property(self, master_gear_efficiency):
        basic_spur_gear_1.master_gear_efficiency = master_gear_efficiency

        assert basic_spur_gear_1.master_gear_efficiency == master_gear_efficiency


    @mark.error
    def test_raises_type_error(self, spur_gear_master_gear_efficiency_type_error):
        with raises(TypeError):
            basic_spur_gear_1.master_gear_efficiency = spur_gear_master_gear_efficiency_type_error


    @mark.error
    def test_raises_value_error(self, spur_gear_master_gear_efficiency_value_error):
        with raises(ValueError):
            basic_spur_gear_1.master_gear_efficiency = spur_gear_master_gear_efficiency_value_error
