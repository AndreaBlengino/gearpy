from .mechanical_object_base import Role


class MatingMaster(Role):
    """:py:class:`MatingMaster <gearpy.mechanical_objects.mating_roles.MatingMaster>` object. \n
    Role of the master gear in a gear mating.
    """


class MatingSlave(Role):
    """:py:class:`MatingSlave <gearpy.mechanical_objects.mating_roles.MatingSlave>` object. \n
    Role of the slave gear in a gear mating.
    """
