...
class SpurGear(GearBase):
    r""":py:class:`SpurGear <gearpy.mechanical_objects.spur_gear.SpurGear>`
    object.
    ...
    def name(self) -> str:
        ...
        powertrain elements. \n
        Once set at the spur gear instantiation, it cannot be changed
        afterward.
    ...
    def reference_diameter(self) -> Optional[Length]:
        ...
        :py:attr:`module` at the spur gear instantiation and it cannot be
        changed afterward.
    ...
    def lewis_factor(self) -> Optional[float]:
        ...
        teeth and the pressure angle. In this case, the considered pressure
        angle is always 20 degrees, so the Lewis factor depends only on the
        number of gear teeth :py:attr:`n_teeth`.
    ...
    def tangential_force_is_computable(self) -> bool:
        ...
        :py:attr:`module`, so if this optional parameter has been set at spur
        gear instantiation, then it is possible to compute the tangential force
    ...
    def bending_stress(self) -> Stress:
        """Bending stress applied on the gear teeth by the mating gear. It must
        be an instance of :py:class:`Stress <gearpy.units.units.Stress>`.
    ...
    def bending_stress_is_computable(self) -> bool:
        ...
        parameters have been set at spur gear instantiation, then it is
    ...
    def compute_contact_stress(self) -> None:
        ...
        .. admonition:: Notes
           ...
           .. math::
               \sigma_c = 0.262922 \, \sqrt{\frac{4 \, F_t}{b \, \cos \alpha \,
               \sin \alpha} \left( \frac{1}{D_1} + \frac{1}{D_2} \right) \,
               \frac{E_1 \, E_2}{E_1 + E_2}}

           where:

           - :math:`F_t` is the :py:attr:`tangential_force` tangential force
             applied on the tooth,
           - :math:`b` is the gear :py:attr:`face_width`,
           - :math:`\alpha` is the pressure angle of the gear, always equal to
             20 degrees,
    ...
    def contact_stress_is_computable(self) -> bool:
        ...
        set at spur gear instantiation, then it is possible to compute the
    ...
    def update_time_variables(self) -> None:
        ...
        if self.tangential_force_is_computable:
            ...
            if self.bending_stress_is_computable:
                self.time_variables['bending stress'].append(
                    self.bending_stress
                )

                if self.contact_stress_is_computable:
                    self.time_variables['contact stress'].append(
                        self.contact_stress
                    )
